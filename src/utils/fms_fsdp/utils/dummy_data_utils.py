...
def split_dataset(
    data_path, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1, saving=True, seed=567
):
    ...
    val_index = shullfed_index[
        int(len(data_json) * train_ratio) : int(
            len(data_json) * (train_ratio + val_ratio)
        )
    ]
    ...
    if saving:
        for json_name, json_data in zip(
            ["train", "val", "test"], [train_dict, val_dict, test_dict]
        ):
