...
logging.basicConfig(level=logging.INFO,format='%(asctime)s - %(name)s - %(levelname)s - %(message)s - %(lineno)%')
...
def main(data_path='datas/train',task_type='0',data_type='train'):
    ...
    for text_prompt,image in tqdm(zip(text_prompt,datas['image']),total=len(text_prompt),desc='inference'):
        inputs = processor(
    ...
    positive_pred=[ 1 for pred,label in zip(processed_output_texts,datas['output']) if pred[0]==label ]
    print(f"positive_pred_tasks_{task_type}:{len(positive_pred)}")
    print(f"output_texts_tasks_{task_type}:{len(processed_output_texts)}")
    print(f"evaluation score_tasks_{task_type}:{len(positive_pred)/len(processed_output_texts)}")
    print(f"saving the prediction results_tasks_{task_type}")
    task='图片分类' if task_type=='0' else '意图分类'
    saving_results={'task':task,'total_num':len(datas['instruction']),'positive_pred':len(positive_pred),'score':len(positive_pred)/len(processed_output_texts)}
    saving_results['id_label_pred_raw_image']=[[id,label,pred[0],raw,image_id] for id,label,pred,raw,image_id in zip(datas['id'],datas['output'],processed_output_texts,raw_output_texts,datas['image_id'])]

    with open(os.path.join('datas/train',f"{data_type}_task_"+task_type+'_results.json'),'w') as f:
